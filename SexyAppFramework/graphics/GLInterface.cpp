...
//	theImageFlags = D3DImageFlag_MinimizeNumSubdivisions;
	...
	if (usePow2)
	{
		...
		{
			int wIndex = aWidth < MAX_TEXTURE_SIZE ? aWidth : (MAX_TEXTURE_SIZE - 1);
			int hIndex = aHeight < MAX_TEXTURE_SIZE ? aHeight : (MAX_TEXTURE_SIZE - 1);
			aWidth = aWidth >= gMaxTextureWidth ? gMaxTextureWidth : aGoodTextureSize[wIndex];
			aHeight = aHeight >= gMaxTextureHeight ? gMaxTextureHeight : aGoodTextureSize[hIndex];
