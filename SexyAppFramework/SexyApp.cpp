...
void SexyApp::ReadFromRegistry()
{
	...
	if (!mPlayingDemoBuffer)
	{
		...
		// aPath;
		//if(GetAppDataFolder().back()=='/')
		std::string	aPath = GetAppDataFolder()+"savedata/popcinfo.dat";
		//else
		//	aPath = GetAppDataFolder()+"/savedata/popcinfo.dat";
		//char aFileName[512] = *(aPath.data());

		FILE* fp = fopen(aPath.data(), "rb");
		if (fp != NULL)
		{
			for (;;)
			{
...
void SexyApp::WriteToRegistry()
{
	...
	if (!mPlayingDemoBuffer)
	{
		...
		//	aPath = GetAppDataFolder()+"/savedata/popcinfo.dat";
		//char* aFileName[512] = {};
		//aFileName*(aPath.data());

		FILE* fp = fopen(aPath.data(), "r+b");
		if (fp != NULL)
		{
			for (;;)
			{
