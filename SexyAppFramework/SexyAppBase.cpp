...
SexyAppBase::SexyAppBase()
{
	...
	//mInvisHWnd = NULL;
	mFrameTime = 10; //TODO make gameplay faster. 5 is twice the speed
	mNonDrawCount = 0;
	mDrawCount = 0;
	mSleepCount = 0;
	mUpdateCount = 0;	
...
*/	
	...
	while (aSharedImageItr != mSharedImageMap.end())
	{
		...
		DBG_ASSERTE(aSharedImage->mRefCount == 0);	 //fixme	
...
void SexyAppBase::Init()
{
	...
	*/
	...
	/*std::string aPath;
	if(configdir.back()=='/')
		aPath = configdir+"savedata/";
	else
		aPath = configdir+"/savedata/";
	SetAppDataFolder(aPath.c_str());*/
...
{	
		//printf("\n%s",theFileName.c_str());fflush(stdout); //ovde sam stavio da printuje za testove
