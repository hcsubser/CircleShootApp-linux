...
CircleShootApp::CircleShootApp()
{
	...
	//mWorkerThread = malloc(sizeof(pthread_t));
	//pthread_create(&mWorkerThread, NULL, WorkerThread::StaticThreadProc, this);
...
void CircleShootApp::LoadingThreadProc()
{
    ...
    for (;;)
    {
        ...
        if (i == 5)
        {
            ...
            Sexy::_Font *main8Font = mResourceManager->GetFont("FONT_MAIN8");
            if (main8Font != NULL)
            {
...
void CircleShootApp::FinishRenameUserDialog(bool confirm)
{
    ...
    if (!aNewName.empty())
    {
        int cmp = strcmp(aSelName.c_str(), aNewName.c_str());
...
void CircleShootApp::SwitchSong(int id)
{
    if (this->mLastSong == id ||
        (clock() - this->mLastSongSwitchTime) >= 5000)
    {
