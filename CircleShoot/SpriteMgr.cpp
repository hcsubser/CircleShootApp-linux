...
void SpriteMgr::SetupLevel(const LevelDesc &theLevel, MirrorType theMirror)
{
    ...
    {
        mHasBackground = true;
		mBackgroundImage = gSexyAppBase->GetImage(theLevel.mImagePath, false); //mozda treba da se rotira, nmp, ce testiram kasnije
		//printf("\n%s",theLevel.mImagePath.c_str());fflush(stdout);
...
void SpriteMgr::DrawBackground(Graphics *g)
{
    ...
    else
    {
		...
		//g->DrawImage(ImageLib::GetImage("levels/levels/spiral",false),0,0,&r);
