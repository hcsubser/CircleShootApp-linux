...
void WayPointMgr::CalcPerpendicularForPoint(int theWayPoint)
{
	//debug stuff
	//if(theWayPoint==0){
	//	printf("\n No waypoints! CircleShoot/WayPoint.cpp, %s",__func__);fflush(stdout);			
	//	return;
	//}
...
bool WayPointMgr::LoadCurve(const std::string &theFile, MirrorType theMirror)
{
    ...
    {
        curvePath += ".dat";
		printf("\n(Debug)<CircleShoot/WayPoint.cpp> %s %s",__func__,curvePath.c_str());
		fflush(stdout);
    }
    ...
    {
		printf("\n(Debug)<CircleShoot/WayPoint.cpp> Could not load curve: %s",curvePath.c_str());
        return false;
    }
