...
void Ball::DrawExplosion(Graphics *g)
{
    ...
    if (mParticles != NULL)
    {
        int aRed = std::min( (int) ((gBrightBallColors[mType] >> 16) & 0xff + 200), 255);
        int aGreen = std::min( (int) ((gBrightBallColors[mType] >> 8) & 0xff + 200), 255);
        int aBlue = std::min( (int) ((gBrightBallColors[mType]) & 0xff + 200), 255);
